...
void Session::CompletedAccept()
{
	PKT_RES_CONNECT resIn;
	resIn.Init();
	resIn.bIsConnect = true;

	this->PostSend(false, resIn.nSize, (char*)&resIn);
}
...
void Session::handle_receive(const boost::system::error_code& error, size_t bytes_transferred)
{
	if (error)
	{
		...
		{
			std::cout << "Client has beed lost." << std::endl;
		}
		else
		{
